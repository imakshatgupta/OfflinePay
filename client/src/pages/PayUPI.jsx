      ...
      {showReceiverModal && (
        <div className="fixed inset-0 flex justify-center items-center bg-gray-800 bg-opacity-50">
          <div className="modal-content bg-white p-4 rounded-md">
            <span className="absolute top-0 right-0 cursor-pointer" onClick={handleReceiverModalClose}>&times;</span>
            <label htmlFor="receiverUpi">Enter Receiver's UPI ID:</label>
            <input
              type="text"
              id="receiverUpi"
              value={paymentData.receiverUpi}
              onChange={(e) => setPaymentData(prevData => ({ ...prevData, receiverUpi: e.target.value }))}
              className="block w-full border-gray-300 rounded-md px-2 py-1 mb-2"
            />
            <button onClick={handleReceiverModalClose} className="bg-blue-500 text-white px-4 py-2 rounded-md">Next</button>
          </div>
        </div>
      )}
      {showAmountModal && (
        <div className="fixed inset-0 flex justify-center items-center bg-gray-800 bg-opacity-50">
          <div className="modal-content bg-white p-4 rounded-md">
            <span className="absolute top-0 right-0 cursor-pointer" onClick={() => setShowAmountModal(false)}>&times;</span>
            ...
            <input
              ...
              className="block w-full border-gray-300 rounded-md px-2 py-1 mb-2"
            />
            <button onClick={handleAmountSubmit} className="bg-blue-500 text-white px-4 py-2 rounded-md">Next</button>
          </div>
        </div>
      )}
      {showPinModal && (
        <div className="fixed inset-0 flex justify-center items-center bg-gray-800 bg-opacity-50">
          <div className="modal-content bg-white p-4 rounded-md">
            <span className="absolute top-0 right-0 cursor-pointer" onClick={() => setShowPinModal(false)}>&times;</span>
            ...
            <input
              ...
              className="block w-full border-gray-300 rounded-md px-2 py-1 mb-2"
            />
            <button onClick={handlePinSubmit} className="bg-blue-500 text-white px-4 py-2 rounded-md">Pay</button>
          </div>
        </div>
      )}
    </>
